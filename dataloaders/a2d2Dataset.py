...
class a2d2Dataset(torch.utils.data.Dataset):
    ...
    def list_images(self):
        ...
        images = []
        path_img = os.path.join(self.opt.dataroot, "camera_lidar_semantic_bboxes")
        for scene in sorted(os.listdir(path_img)):
            cur_folder = os.path.join(path_lab, scene, 'camera/cam_front_center')
            for item in sorted(os.listdir(cur_folder)):
                if item.find(".png") != -1:
                    images.append(os.path.join(scene, item))
        labels = []
        #path_lab = os.path.join(self.opt.dataroot, "labels/sem_seg/colormaps", mode)
        path_lab = os.path.join(self.opt.dataroot, "camera_lidar_semantic_bboxes")
        for scene in sorted(os.listdir(path_lab)):
            cur_folder = os.path.join(path_lab, scene, 'label/cam_front_center')
            for item in sorted(os.listdir(cur_folder)):
                labels.append(item)
